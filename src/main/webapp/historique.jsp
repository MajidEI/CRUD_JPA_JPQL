...
<div class='container mt-4'>
    ...
    <table style="width: 80%" class="table table-bordered">
        ...
        <c:forEach items="${requestScope.historique}" var="commande">
            <tr>
                <th scope="row">${commande.key.numCommande}</th>
                <td>${commande.key.dateCommande}</td>
                <td>${commande.key.client.nom} ${commande.key.client.prenom}</td>
                <td>
                    <ul>
                        <c:forEach items="${commande.value}" var="article">
                            <li>${article.article.designation}-${article.article.prix} (x ${article.qteCde})</li>
                        </c:forEach>
                    </ul>
                </td>
            </tr>
